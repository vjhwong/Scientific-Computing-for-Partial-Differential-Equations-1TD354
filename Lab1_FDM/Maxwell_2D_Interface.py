...
if bc_type == 1:
    ...
    Lbc = bmat(
        [
            [Lbc1, z_2m_N, z_2m_N, z_2m_N],
            [z_2m_N, Lbc2, z_2m_N, z_2m_N],
            [z_2m_N, z_2m_N, Lbc3, z_2m_N],
            [z_2m_N, z_2m_N, z_2m_N, Lbc4],
        ],
        format="csc",
    )
elif bc_type == 2:
    ...
    Lbc = bmat(
        [
            [Lbc1, z_2mm2_N, z_2mm2_N, z_2mm2_N],
            [z_2mm2_N, Lbc2, z_2mm2_N, z_2mm2_N],
            [z_2mm2_N, z_2mm2_N, Lbc3, z_2mm2_N],
            [z_2mm2_N, z_2mm2_N, z_2mm2_N, Lbc4],
        ],
        format="csc",
    )
elif bc_type == 3:
...
y0 = 0.5
Ex1 = np.zeros(mtot)
Hz1 = np.exp(-((X1 - x0) ** 2) / rstar**2 - (Y1 - y0) ** 2 / rstar**2).reshape(
    mtot, order="F"
)
Ey1 = np.zeros(mtot)

Ex2 = np.zeros(mtot)
Hz2 = np.exp(-((X2 - x0) ** 2) / rstar**2 - (Y2 - y0) ** 2 / rstar**2).reshape(
    mtot, order="F"
)
Ey2 = np.zeros(mtot)

Ex3 = np.zeros(mtot)
Hz3 = np.exp(-((X3 - x0) ** 2) / rstar**2 - (Y3 - y0) ** 2 / rstar**2).reshape(
    mtot, order="F"
)
Ey3 = np.zeros(mtot)

Ex4 = np.zeros(mtot)
Hz4 = np.exp(-((X4 - x0) ** 2) / rstar**2 - (Y4 - y0) ** 2 / rstar**2).reshape(
    mtot, order="F"
)
Ey4 = np.zeros(mtot)
...
srf1 = ax.pcolor(
    X1, Y1, Hz1.reshape((m, m), order="F"), shading="nearest", vmin=zlow, vmax=zhigh
)
srf2 = ax.pcolor(
    X2, Y2, Hz2.reshape((m, m), order="F"), shading="nearest", vmin=zlow, vmax=zhigh
)
srf3 = ax.pcolor(
    X3, Y3, Hz3.reshape((m, m), order="F"), shading="nearest", vmin=zlow, vmax=zhigh
)
srf4 = ax.pcolor(
    X4, Y4, Hz4.reshape((m, m), order="F"), shading="nearest", vmin=zlow, vmax=zhigh
)
